...
def evaluate_model_scoring(classifier, X, y, cv):
    ...
    scores = cross_validate(
        estimator=classifier, X=X, y=y, cv=cv, scoring=scoring, n_jobs=-1
    )
...
def evaluate_model_cm(clf, X, y, cv, cm_name="conf-mat-agg"):
    ...
    print(
        classification_report(
            actual_labels_accum, predicted_labels_accum, target_names=clf.classes_
        )
    )
    cr = classification_report(
        actual_labels_accum,
        predicted_labels_accum,
    ...
    disp = ConfusionMatrixDisplay(
        confusion_matrix=conf_mat_accum.astype(int), display_labels=clf.classes_
    )
    ...
    # plt.show()
    plt.savefig(
        test_classifiers_figs_dir()
        + "/"
        + str(y.nunique())
        + "-class-confusion-map.png"
    )
...
def grid_search(classifier, param_grid, X, y):
    optimal_params = GridSearchCV(
        classifier, param_grid, cv=5, scoring="f1_micro", verbose=0, n_jobs=-1
    )
