...
if execute == 14:
    ...
    with keep.running() as k:
        ...
        raw_data.to_csv(
            external_data_dir() + "/" + "raw_data_std_denom_var.csv", index=False
        )
    ...
    with keep.running() as k:
        ...
        test_classifiers_basic(
            ...
            scoring=False,
            cm=True,
            cm_name="conf-mat-agg_post_lasso",
        )
    ...
    with keep.running() as k:
        raw_data = pd.read_csv(
            external_data_dir() + "/" + "raw_data_prepared.csv", index_col=0
        )
    ...
    with keep.running() as k:
        model = mpu.io.read(
            external_data_dir() + "/" + "conf-mat-agg_post_lasso.pickle"
        )
        model1 = mpu.io.read(
            external_data_dir() + "/" + "conf-mat-agg_post_corr.pickle"
        )
    ...
    with keep.running() as k:
        results = pd.read_pickle(
            external_data_dir() + "/" + "dt_params.pkl", compression="infer"
        )
        print(results)
